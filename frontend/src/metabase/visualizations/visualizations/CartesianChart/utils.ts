...
export const getDimensionsFromECharts = (
  echartsEvent: any,
  chartModel: CartesianChartModel,
) => {
  const { dataIndex, seriesIndex } = echartsEvent;
...
export const getDataFromEChartsEvent = (
  echartsEvent: any,
  chartModel: CartesianChartModel,
) => {
  const { dataIndex, seriesIndex } = echartsEvent;
  ...
  const eventData = getObjectEntries(datum)
    .map(([dataKey, value]) => {
      ...
      return {
        key: col.name.display_name, // TODO: use the title from the viz settings
        value,
        col,
      };
    })
